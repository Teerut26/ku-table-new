...
const ListSubject: NextPage<Props> = () => {
  ...
  const { sectionDay, sectionStudentType, sectionType, setResult, sectionTime } = useFilterStore((r) => r);
  const [subjectsDataTemp, setSubjectsDataTemp] = useState<OpenSubjectForEnrollInterface[]>([]);
  ...
  }, [subjectsApi.data, sectionDay, sectionStudentType, sectionType, sectionTime]);
  ...
  const Filter = (input: OpenSubjectForEnrollInterface[]) => {
    try {
      ...
      let sectionTypeFilted = sectionType.length === 0 ? input : sectionTypeFiltedRaw;

      const sectionStudentTypeFiltedRaw = sectionTypeFilted.filter((subject) => {
        return sectionStudentType.includes(subject.stdStatusEn as any);
      });

      let sectionStudentTypeFilted = sectionStudentType.length === 0 ? sectionTypeFilted : sectionStudentTypeFiltedRaw;

      const sectionDayFiltedRaw = sectionStudentTypeFilted.filter((subject) => {
        const daySeparate = CourseDateSeparate(subject.coursedate).map((day) => day.day_w);
        return _.intersection(sectionDay, daySeparate).length > 0;
      });

      let sectionDayFilted = sectionDay.length === 0 ? sectionStudentTypeFilted : sectionDayFiltedRaw;

      const sectionTimeFiltedRaw = sectionDayFilted.filter((subject) => {
        const daySeparate =
          CourseDateSeparate(subject.coursedate).filter((day) => {
            return isTimeInRanges(sectionTime?.timeFrom!, sectionTime?.timeTo!, day.time_from!, day.time_to!);
          }).length > 0;
  ...
  return (
    <div className="flex flex-col gap-3">
      {subjectsApi.data && subjectsApi.data.results.length > 0 ? (
        subjectsDataTemp.map((subject, index) => <Subject subject={subject} key={index} classe={classes.find((c) => c.classId === subject.subjectCode.split("-")[0])} />)
      ) : (
        <div className="text-center my-auto">{LocalsSwip("ไม่พบรายวิชาที่ค้นหาหรือยังไม่เปิด", "No subject found or not open yet")}</div>
      )}
    </div>
  );
};
