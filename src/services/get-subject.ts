...
const getSubject = async (props: Props) => {
  return axios<BaseApiStructure<OpenSubjectForEnrollInterface[]>>({
    method: "get",
    url: CORSProxy(`https://myapi.ku.th/enroll/openSubjectForEnroll?query=${props.query}&academicYear=${props.academicYear}&semester=${props.semester}&campusCode=${props.campusCode}&section=${props.section}`),
    headers: {
